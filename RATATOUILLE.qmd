...
```{r, include=FALSE}
sentiment_review|>
  filter(sentiment_review<=0)

bad_reviews<-str_detect(user_reviews, "cry|bored|hard|struggling|critic|leave")

print(user_reviews[bad_reviews])

review_table<-data.frame(print(user_reviews[bad_reviews]))


kable(review_table, format = "html", col.names = c("Review with negative? word")) |>
  kable_styling(full_width = FALSE)
