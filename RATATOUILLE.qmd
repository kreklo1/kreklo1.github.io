...
```{r disney_movie_lengthss, echo=FALSE, message=FALSE, warning=FALSE}
...
observed_statistic <- disney_movies$`Box office (float)`[disney_movies$title == "Ratatouille"]
num_iterations <- 1000
simulated_statistics <- numeric(num_iterations)

for (i in 1:num_iterations) {
  random_box_office <- sample(disney_movies$`Box office (float)`)
  new_test_stat <- random_box_office
  simulated_statistics[i] <- new_test_stat
}

p_value <- mean(simulated_statistics >= observed_statistic)
print(p_value)

simulated_df <- data.frame(simulated_statistic = simulated_statistics)

ggplot(simulated_df, aes(x = simulated_statistic)) +
  geom_histogram(color = "pink2", fill = "lightpink", bins = 30) +
  geom_vline(xintercept = observed_statistic, color = "hotpink", linewidth = 1) +
  labs(title = "Ratatouille Observed Performance vs. Simulated Statistics of Disney Movies",
