...
simulated_stats <- function(data, variable, n_permutations) {
  ...
  for (i in 1:1000) {
    ...
    simulated_diffs[i, 1] <- mean(permuted_data$Gain[permuted_data$Source == "Beef"]) -
                                  mean(permuted_data$Gain[permuted_data$Source == "Pork"])
    
    simulated_diffs[i, 2] <- mean(permuted_data$Gain[permuted_data$Source == "Beef"]) -
                                  mean(permuted_data$Gain[permuted_data$Source == "Cereal"])
    
    simulated_diffs[i, 3] <- mean(permuted_data$Gain[permuted_data$Source == "Pork"]) -
                                  mean(permuted_data$Gain[permuted_data$Source == "Cereal"])
  }
...
ggplot(null_world_beef_cereal, aes(x = simulated_diffs)) +
  ...
  labs(x = "Difference in means (Beef vs. Cereal)")
...
ggplot(null_world_pork_cereal, aes(x = simulated_diffs)) +
  ...
  labs(x = "Difference in means (Pork vs. Cereal)")
cat("Pork vs. Cereal p-value:", p_values[3], "\n")

```

As we can see, all of the p-values are close to 0.3. Generally, if the p-value is less than 0.05, we will consider the results of the study statistically significant. Because these results are well over the point of statistical significance, we can fail to reject the null hypothesis and claim that the difference in weight gains due to different protiens is not statistically significant.